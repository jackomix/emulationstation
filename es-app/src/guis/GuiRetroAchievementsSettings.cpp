...
GuiRetroAchievementsSettings::GuiRetroAchievementsSettings(Window* window, bool setCursorToOnlineMode) : GuiSettings(window, _("RETROACHIEVEMENT SETTINGS").c_str())
{
	addGroup(_("SETTINGS"));
	...
	addSaveFunc([username, password, window]
	{
		...
		if (username != newUsername || password != newPassword || token.empty())
		{
			std::string mode = Settings::getInstance()->getString("RetroachievementsOfflineMode");
			bool isOffline = (mode == "always_offline") || (mode == "auto" && ApiSystem::getInstance()->getIpAddress() == "NOT CONNECTED") || (mode.empty() && ApiSystem::getInstance()->getIpAddress() == "NOT CONNECTED");
		...
		if (SystemConf::getInstance()->setBool("global.retroachievements", true))
